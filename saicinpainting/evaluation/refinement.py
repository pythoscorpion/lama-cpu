...
def _infer(
    ...
    ekernel = ekernel.to('cpu')
    image = image.to('cpu')
    z1, z2 = z1.detach().to('cpu'), z2.detach().to('cpu')
    ...
    for idi in pbar:
        ...
        for idd, forward_rear in enumerate(forward_rears):
            ...
            if idd < len(devices) - 1:
                ...
                midz1, midz2 = midz1.to('cpu'), midz2.to('cpu')
...
def refine_predict(
    ...
    for idd in range(len(gpu_ids)):
        ...
        #forward_rears[idd].to(devices['cpu'])
        forward_rears[idd].to('cpu') 
    ...
    for ids, (image, mask) in enumerate(zip(ls_images, ls_masks)):
        ...
        image, mask = move_to_device(image, 'cpu'), move_to_device(mask, 'cpu')

        if image_inpainted is not None:
            image_inpainted = move_to_device(image_inpainted, 'cpu')
